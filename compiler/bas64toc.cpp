...
private:
    ...
    void generateVariableDeclarations() {
        ...
        for (const auto& var : variables) {
            outFile << "long long " << var << " = 0;\n";
        }
        for (const auto& arr : arrays) {
            if (arrayDimensions.find(arr) != arrayDimensions.end()) {
                outFile << "long long " << arr << "[" << arrayDimensions[arr] << "];\n";
            } else {
                outFile << "long long " << arr << "[1000];\n";  // Default size
            }
        }
        outFile << "\n";
    }
    ...
    void translatePrint(const std::vector<Token>& tokens, size_t startIndex) {
        ...
        while (i < tokens.size()) {
            if (tokens[i].type == TokenType::STRING) {
                ...
                outFile << "    printf(\"%lld\", " << translateExpression(tokens, i, exprEnd) << ");\n";
                i = exprEnd;
            }
        }
